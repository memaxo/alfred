...
- [ ] Install and validate missing Codex SDK + Playwright harness assets (`bun pm ls @openai/codex-sdk`, `bunx playwright install --with-deps chromium`), document credentials/env requirements in `config/env.example`, and run smoke Playwright suites gated behind proper env flags.
...
- **Codex SDK + Playwright harness**: `packages/agent/src/orchestrator/tool/codex/exec.ts` already imports `@openai/codex-sdk`, but local devs may not have the bundled binary installed or `CODEX_API_KEY` configured. `config/env.example` documents `CODEX_BACKEND` (line ~189). Playwright tests (apps/web/playwright.config.ts) rely on `bunx playwright test` and require Chromium browsers installed via `bunx playwright install`. Missing dependencies currently force CI to skip large suites.
...
4. **Install Codex SDK + Playwright harness**: Verify `@openai/codex-sdk` presence via `bun pm ls @openai/codex-sdk`. If missing, run `bun add @openai/codex-sdk`. Install Codex binary assets if required (per SDK README). Update `config/env.example` with any new env vars needed (`CODEX_API_KEY`, `CODEX_BACKEND=sdk`, `PLAYWRIGHT_SKIP_WEB_SERVER`). Run `bunx playwright install --with-deps chromium` so tests have browsers. Execute smoke Playwright suite (`bunx playwright test --config apps/web/playwright.config.ts --project=smoke`) and targeted Codex tests (`bun test packages/agent/test/codex-exec.test.ts`) to confirm harness works; record any blocked suites and missing credentials.
...
- **Toolchain**: `bun pm ls @openai/codex-sdk` confirms installation, Playwright smoke project passes locally, and doc updates explain any remaining blocked suites.
...
- **Codex SDK (`@openai/codex-sdk`)**: Required by `packages/agent/src/orchestrator/tool/codex/exec.ts`; confirm via `bun pm ls` and ensure `CODEX_API_KEY` plus `CODEX_SESSION_DIR` envs exist (see `config/env.example`).
