...
16. **Fixture-driven tests.** Any test that touches `VoiceRegistry`, pools, or the streaming prototype must install the shared fixture from `@alfred/test-kit/voice/runtime-fixture`, letting the real WebSocket server run while only configuring transcripts/chunks via fixture options and cleaning up with `restore()`—never replace `@alfred/voice` modules with ad-hoc mocks.
