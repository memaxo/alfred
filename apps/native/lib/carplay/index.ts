...
export function setupCarPlay(voice: VoiceBridge, onReply: (text: string) => void = noop): void {
	if (Platform.OS !== "ios") {
		return;
	}
	...
	const buildTemplate = () => {
		...
		const VoiceControlButton = carplay.VoiceControlButton ?? carplay.CarPlayButton;
