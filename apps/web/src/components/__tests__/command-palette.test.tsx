...
mock.module("@/components/ui/command", () => ({
  ...
  CommandInput: ({ placeholder, onValueChange, onKeyDown, value, suggestion }: any) => (
    <div>
      <input 
  ...
  CommandEmpty: () => null,
  CommandGroup: ({ heading, children }: any) => <div><h3>{heading}</h3>{children}</div>,
  CommandItem: ({ children, onSelect }: any) => <li onClick={onSelect}>{children}</li>,
...
describe("MindscapeCommandPalette", () => {
  ...
  it("should open on Cmd+K", async () => {
    render(<MindscapeCommandPalette nodes={[]} onFocus={mock()} onSpawn={mock()} />);
