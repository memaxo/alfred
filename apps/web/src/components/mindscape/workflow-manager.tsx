...
function WorkflowSubscription({ nodeId, data }: { nodeId: string, data: ArtifactData }) {
  ...
  // Subscription
  trpc.workflow.stream.useSubscription(streamInput!, {
    enabled: !!streamInput,
    onData(event: WorkflowEvent) {
       ...
       if (event.type === "run") {
         ...
         updateArtifactData(nodeId, { status: "failed", error: (event as any).message });
         setStreamInput(null);
       }
...
export function WorkflowManager() {
  ...
  const activeWorkflowNodes = useMindscapeStore(useShallow((state) => 
