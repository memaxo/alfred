...
type MindscapeEventType = "voice-input" | "voice-output" | "tool-call" | "rag-retrieval" | "workflow-step";
...
export function dispatchMindscapeEvent(event: MindscapeEvent) {
  const customEvent = new CustomEvent("mindscape-activation", { detail: event });
  mindscapeEventBus.dispatchEvent(customEvent);
}

export function useMindscapeActivations() {
  const triggerNodeActivity = useMindscapeStore((state) => state.triggerNodeActivity);
  const triggerEdgeActivity = useMindscapeStore((state) => state.triggerEdgeActivity);
  ...
  const handleEvent = useCallback((e: Event) => {
    ...
    if (detail.sourceId && detail.targetId) {
       const edge = edges.find(
          edge => 
             (edge.source === detail.sourceId && edge.target === detail.targetId) ||
  ...
  useEffect(() => {
    ...
    return () => {
      mindscapeEventBus.removeEventListener("mindscape-activation", handleEvent);
    };
  }, [handleEvent]);
