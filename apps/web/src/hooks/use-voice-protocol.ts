...
import type { VoiceStreamClient, VoiceStreamClientHandlers } from "@alfred/voice/stream";
import type { VoiceStreamCodec, VoiceStreamServerEvent } from "@alfred/type/voice";
import { getVoiceStreamUrl } from "@/utils/voice-stream";

export type VoiceProtocolState = {
  status: "idle" | "connecting" | "recording" | "processing" | "playing" | "error";
  transcript: string;
...
export function useVoiceProtocol(
  ...
  handlers: {
    onAudioChunk: (chunk: Extract<VoiceStreamServerEvent, { type: "tts_chunk" }>) => void;
    onInterrupt: () => void;
  }
) {
  ...
  const streamHandlers = useRef<VoiceStreamClientHandlers>({
    ...
    onPartialTranscript: (event) => setState((prev) => ({ ...prev, transcript: event.text })),
    onFinalTranscript: (event) => setState((prev) => ({ ...prev, transcript: event.text })),
    onVadState: (event) => setState((prev) => ({ ...prev, vadConfidence: event.vadConfidence ?? null })),
    onAutoStop: (event) => {
      setState((prev) => ({ ...prev, autoStopReason: event.reason, status: "processing" }));
    },
    onAssistantMessage: (event) => setState((prev) => ({ ...prev, assistantText: event.text })),
    ...
    onError: (event) => setState((prev) => ({ ...prev, status: "error", error: event.message })),
  }).current;
  ...
  const disconnect = useCallback(async (reason: "manual" | "silence" | "timeout" = "manual") => {
    try {
