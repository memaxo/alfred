...
describe("layoutSemantic", () => {
  ...
  it("should respect focus gravity", () => {
    ...
    const dist2 = Math.sqrt(Math.pow(n2.position.x - n1.position.x, 2) + Math.pow(n2.position.y - n1.position.y, 2));
    const dist3 = Math.sqrt(Math.pow(n3.position.x - n1.position.x, 2) + Math.pow(n3.position.y - n1.position.y, 2));
