...
function suggestProdHost(app: string) {
	...
	const domain =
		...
		(window.location.hostname.length > 0 ? window.location.hostname : DEFAULT_DOMAIN_FALLBACK);
	return `${slug}.${domain}`;
}
...
function DeploymentsRoute() {
	...
	const [healthStatusMap, setHealthStatusMap] = useState<Record<string, { status: "healthy" | "unhealthy" | "unknown"; url: string | null; ts: string }>>({});
	...
	useEffect(() => {
		...
		(async () => {
			try {
				...
				if (!cancelled) {
					const message = error instanceof Error ? error.message : "Failed to start live health";
					toast.error(message);
					setLiveHealth(false);
				}
			}
		...
		setHealthStatusMap(prev => {
			...
			for (const deployment of deployments) {
				...
				if (!next[key] && deployment.healthStatus) {
					next[key] = {
						status: (deployment.healthStatus as "healthy" | "unhealthy" | "unknown") ?? "unknown",
	...
	async function handleRemove(app: string, preview: boolean) {
		if (!window.confirm(`Remove the ${preview ? "preview" : "production"} deployment for ${app}?`)) {
			return;
		}
		try {
			...
			const message = error instanceof Error ? error.message : "Unable to remove deployment";
			toast.error(message);
		}
	}
		...
		<div className="container mx-auto max-w-5xl space-y-6 px-4 py-6">
			<Card>
				<CardHeader>
					...
					<CardDescription>Preview and production deployments created by Alfred.</CardDescription>
				</CardHeader>
				<CardContent>
					{isLoading ? (
						...
						<p className="text-sm text-muted-foreground">
							No deployments yet. Use the orchestrator or API to provision a preview build.
						</p>
					) : (
						...
						<thead className="bg-muted/50 text-left">
							...
							{deployments.map((deployment) => {
								...
								const derivedStatus = (liveEntry?.status ?? (deployment.healthStatus as "healthy" | "unhealthy" | "unknown" | null) ?? "unknown") as "healthy" | "unhealthy" | "unknown";
								...
								const healthTimestamp = liveEntry?.ts ?? (deployment.lastHealthCheck ? new Date(deployment.lastHealthCheck).toISOString() : null);
								return (
									<tr className="border-t border-border" key={deployment.id}>
										<td className="px-3 py-2">{deployment.app}</td>
										<td className="px-3 py-2 capitalize">{deployment.type}</td>
										<td className="px-3 py-2">
											<span className="font-medium text-foreground">{stateLabel}</span>
										</td>
										<td className="px-3 py-2">
											<div className="flex flex-col">
												<span className={`font-medium ${healthClass}`}>{healthLabel}</span>
												{healthTimestamp ? (
			...
			<PromoteDialog
				...
				onSubmit={async ({ upstream, host }) => {
					...
					try {
						...
						const message = error instanceof Error ? error.message : "Unable to promote deployment";
						toast.error(message);
					}
				}}
			/>
