...
function PreferencesRoute() {
	...
	const setPreference = trpc.preference.set.useMutation({
				...
				const optimisticEntry =
					existingIndex >= 0
						? {
								...base[existingIndex],
								value: input.value,
								confidence: input.confidence ?? base[existingIndex]?.confidence ?? 1,
		...
		onError: (error, _input, context) => {
			utils.preference.list.setData(listInput, context?.previous as PreferenceList);
			toast.error(error.message ?? "preference_update_failed");
		},
		onSuccess: (data) => {
			utils.preference.list.setData(listInput, (current) => {
				const base = Array.isArray(current) ? [...current] : [];
				const index = base.findIndex((item) => item?.id === data.id || item?.key === data.key);
				if (index >= 0) {
					base[index] = data;
	...
	const handleSubmit = useCallback(
		(event: FormEvent<HTMLFormElement>) => {
			...
			const rawConfidence = parseFloat(form.confidence);
								...
								>
									<div className="flex items-start justify-between gap-4">
										<div className="space-y-2">
											<h3 className="text-base font-semibold">{preference.key}</h3>
