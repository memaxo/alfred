...
export const RAG_DOC_CACHE_LIMIT = resolvePositiveNumber(
  import.meta.env?.VITE_DESKTOP_RAG_CACHE_LIMIT,
  50
);

export const RAG_DOC_CACHE_TTL_MS = resolvePositiveNumber(
  import.meta.env?.VITE_DESKTOP_RAG_CACHE_TTL_MS,
  5 * 60 * 1000
);
...
export type CacheSlice = {
  ...
  cacheRagDoc: (dbId: string, data: WindowData & { type: "knowledge" }) => void;
  ...
  cacheRagDoc: (dbId, data) => {
    ...
    set((state) => {
      ...
      if (keys.length > RAG_DOC_CACHE_LIMIT) {
        keys
          .sort((a, b) => (next[a]?.cachedAt ?? 0) - (next[b]?.cachedAt ?? 0))
