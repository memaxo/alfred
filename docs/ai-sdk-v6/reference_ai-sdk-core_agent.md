...
`generate()`| `PromiseLike>`| Generates full, non-streaming output for a text prompt or messages.  
`stream()`| `StreamTextResult>`| Streams output (chunks or steps) for a text prompt or messages.  
...
## See Also
  ...
  * `ToolLoopAgent` — Official multi-step agent implementation
  * `createAgentUIStream`
  * `GenerateTextResult`
  * `StreamTextResult`
