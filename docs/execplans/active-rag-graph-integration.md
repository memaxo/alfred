...
Upgrade the "Active RAG" system from purely semantic (vector-based) retrieval to **Graph-Aware Retrieval**. This ensures ALFRED understands not just the _topic_ of a focused node, but its _structural place_ in the system (dependencies, blockers, parent/child relations), enabling truly "Cognitive" context.
...
- **UI Feedback**: The Context Lens now explicitly shows _why_ something is in context (e.g., "Is DEPENDS_ON by..."), reinforcing the user's mental model of the graph.
