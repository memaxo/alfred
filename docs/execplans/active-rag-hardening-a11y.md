...
1.  **Keyboard Navigation**:
    ...
    - _Note_: Radix `HoverCard` does not open on focus by default. We may need to switch to `Popover` or compose them, OR accept that it's a "progressive enhancement" and ensure the info is available elsewhere.
    - _Better approach_: Since this is "Hover" card, for A11y, we should probably wrap it or use `Tooltip` patterns. However, given it's interactive content (scrollable list), a `Popover` triggered by click is often better for A11y than a HoverCard.
    ...
    - _Alternative_: Keep `HoverCard` but ensure the trigger has `aria-label` summarizing the context count.
