...
2.  **Hook Integration**:
    ...
    *   Pass the *debounced* value to the `trpc.graph.runQuery.useQuery` hook key/enabled check.

3.  **Visual Feedback**:
    *   Ensure the UI doesn't flash "Loading" immediately upon focus change if we are in the debounce window. The `ContextLens` should likely retain the *previous* context or show a neutral state until the debounce timer clears.
