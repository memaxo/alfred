...
Currently, focusing a node in Mindscape is purely visual. It changes the layout and highlights the node, but if the user starts typing in the chat/command palette, the AI doesn't inherently know _which_ node is focused or what its content is. We need to bridge this gap.
...
- **Inject**: Add a transparent "System Message" or "Context Block" to the AI's prompt.
  - _User Focus: Node "Project X" (Type: Note)_
  - _Content: ..._
...
2.  [x] **Context Hook**: Create `useFocusedContext()`:
    ...
    - _Update_: Handle `ChatNode` focus by looking for connected neighbors (context propagation).
