...
## Decision Log
- **Decision**: Move reranking logic *out* of `db/repo/rag.ts` into `KnowledgeEngine`.
    - *Reason*: Repositories should be pure data access; complex ML orchestration belongs in the engine layer.
- **Decision**: Use `plainto_tsquery` for boost concepts.
    - *Reason*: Prevents SQL injection/syntax errors from user-generated concept strings (e.g., "C++" vs "C++ 20").
