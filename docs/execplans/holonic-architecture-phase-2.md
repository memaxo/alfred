...
- **Learning**: If the user teaches ALFRED "Bun is a runtime", and ALFRED knows "Runtimes are Coding", ALFRED _learns_ that "Bun" implies "Coding" without a code update.
- **Performance**: Regex/Vectors run on _every_ message. Graph query runs only when adaptation is needed (and is efficiently indexed).
