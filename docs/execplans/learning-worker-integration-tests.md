...
- The concurrency test relies on the fact that async operations in the worker (processing runs and maintenance) happen sequentially in the event loop tick but within the same `setInterval` callback. We verified that *both* effects occur.
