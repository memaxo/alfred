...
- Simplifies `packages/agent` to focus purely on _Assistant_ logic (chat, persona), not _Autonomous_ logic (loops).
...
**Recommendation:** Phase this in. Start by moving `agent/src/orchestrator` code _into_ `runtime/src/orchestrator` to colocate it, making `runtime` the definitive owner of autonomous loops. `agent` should just be the "personality" layer.
...
The current implementation spawns a `toolCodex` agent just to _say_ it's going to run tests. This adds ~10 seconds of latency and input token costs for zero decision-making value. The `ReviewPlan` object already contains the intent.
