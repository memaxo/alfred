...
1.  **Create Verification Script**: Create `scripts/verify-stt-streaming.ts`.
    - This should be a standalone script (like the other verify scripts) that spawns the _real_ Python process (if available) or a specialized mock that mimics the VAD chunking behavior.
