...
```graphql
query Issues {
  issues(filter: { description: { null: true } }) {
    nodes {
      id
      title
      description
...
```graphql
query Issues {
  issues(filter: { priority: { eq: 1 }, dueDate: { lte: "2021" } }) {
    nodes {
      id
      title
      priority
...
```graphql
query Issues {
  issues(filter: { labels: { every: { name: { eq: "Bug" } } } }) {
    nodes {
      id
      title
    }
...
query Issues {
  viewer {
    assignedIssues(filter: { comments: { body: { contains: "👍" } } }) {
      nodes {
        id
        title
      }
