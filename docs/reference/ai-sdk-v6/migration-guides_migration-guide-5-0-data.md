Migration GuidesMigrate Your Data to AI SDK 5.0
...
4. Update the rest of your application code to AI SDK 5.0 (see the main migration guide)
...
Create type guards to detect which message format you're working with, and build a conversion function that handles all v4 message types:
    ...
    type V4Part = NonNullable[number];
    ...
    type V4ToolInvocationPart = Extract;




    type V4ReasoningPart = Extract;




    type V4SourcePart = Extract;




    type V4FilePart = Extract;
...
Once migration is complete, update your read functions to use the new v5 schema. Since the data is now in v5 format, you don't need conversion:
    ...
    export const loadChat = async (chatId: string): Promise => {
