...
(options: TransformOptions) => TransformStream, TextStreamPart>
...
(options: PrepareStepOptions) => PrepareStepResult | Promise>
...
(options: ToolCallRepairOptions) => Promise
...
(event: OnChunkResult) => Promise |void
...
(result: onStepFinishResult) => Promise | void
...
(result: OnFinishResult) => Promise | void
...
(event: OnAbortResult) => Promise | void
...
A stream of partial parsed outputs. It uses the `output` specification. AsyncIterableStream is defined as AsyncIterable & ReadableStream.
...
(options?: ConsumeStreamOptions) => Promise
...
(options?: UIMessageStreamOptions) => AsyncIterableStream
...
(options: { part: TextStreamPart & { type: "start" | "finish" | "start-step" | "finish-step"; }; }) => unknown
