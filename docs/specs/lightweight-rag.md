...
-   **Default (Dev)**: `nomic-ai/nomic-embed-text-v1.5` (137M params) or `BAAI/bge-m3` (567M params).
    -   *Challenge*: These models typically output 768 or 1024 dimensions. `nomic-embed-text-v1.5` supports Matryoshka/variable dimensions (64 to 768).
    -   *Conflict*: Our DB schema (`rag.ts`) is hardcoded to `VECTOR_DIM = 1024` (derived from `KaLM`).
    -   *Resolution*: Use `Snowflake/snowflake-arctic-embed-l` (1024 dim support) OR keep `KaLM` but enforce `EMBED_POOL_SIZE=1` and add quantization support (4-bit/8-bit) in `embed_server.py`.

    *Decision*: **Quantization + Single Worker** is the safest path to keep the 1024-dim schema constant without complex migration logic.
