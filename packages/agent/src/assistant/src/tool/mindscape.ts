...
export const toolMindscapeRead = {
  name: "mindscape_read",
  description: "Read the current state of the Mindscape (nodes and connections). Use this to understand the spatial layout of knowledge.",
  inputSchema: z.object({
    query: z.string().optional().describe("Optional search query to filter nodes by label"),
  }),
  execute: async ({ query }: { query?: string }) => {
    ...
    return {
      nodes: filteredNodes.map(n => ({ id: n.id, label: n.label, kind: n.kind })),
...
export const toolMindscapeConnect = {
  ...
  execute: async ({ fromId, toId, kind }: { fromId: string; toId: string; kind: "relates_to" | "blocks" | "depends_on" }) => {
    const [edge] = await db.insert(memoryEdges).values({
      fromId,
      toId,
      kind,
      resource: "agent",
