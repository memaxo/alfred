...
import { proxmox, type ProxmoxError } from "./proxmox";
...
test("lxcCreate sends correct request", async () => {
	const fetchMock = mock((url: string, init?: RequestInit) => {
		...
		return Promise.resolve(new Response(JSON.stringify({ data: { upid: "UPID:test" } }), { status: 200 }));
	});
	// @ts-expect-error override in tests
	globalThis.fetch = fetchMock as typeof globalThis.fetch;

	const client = new proxmox({ base: mockBase, tokenId: mockTokenId, tokenSecret: mockTokenSecret });
...
test("lxcStart sends correct request", async () => {
	const fetchMock = mock(() =>
		Promise.resolve(new Response(JSON.stringify({ data: { upid: "UPID:start" } }), { status: 200 })),
	);
	// @ts-expect-error override in tests
	globalThis.fetch = fetchMock as typeof globalThis.fetch;
...
test("taskWait polls until stopped", async () => {
	...
	const fetchMock = mock(() => {
		callCount++;
		if (callCount < 3) {
			return Promise.resolve(new Response(JSON.stringify({ data: { status: "running" } }), { status: 200 }));
		}
		return Promise.resolve(
			new Response(JSON.stringify({ data: { status: "stopped", exitstatus: "OK" } }), { status: 200 }),
		);
	});
	// @ts-expect-error override in tests
	globalThis.fetch = fetchMock as typeof globalThis.fetch;
...
test("taskWait times out", async () => {
	const fetchMock = mock(() =>
		Promise.resolve(new Response(JSON.stringify({ data: { status: "running" } }), { status: 200 })),
	);
	// @ts-expect-error override in tests
	globalThis.fetch = fetchMock as typeof globalThis.fetch;
	...
	try {
		await client.taskWait("pve", "UPID:test", { timeoutMs: 50, intervalMs: 10 });
...
test("401 error maps to auth error", async () => {
	const fetchMock = mock(() =>
		Promise.resolve(new Response(JSON.stringify({}), { status: 401, statusText: "Unauthorized" })),
	);
	// @ts-expect-error override in tests
	globalThis.fetch = fetchMock as typeof globalThis.fetch;
...
test("500 error maps to server error", async () => {
	const fetchMock = mock(() =>
		Promise.resolve(new Response(JSON.stringify({}), { status: 500, statusText: "Internal Server Error" })),
	);
	// @ts-expect-error override in tests
	globalThis.fetch = fetchMock as typeof globalThis.fetch;
