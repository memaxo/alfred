...
const vmPowerSchema = z.enum(["start", "stop", "reset", "shutdown", "suspend", "resume"]);
...
type StatusResponse = { status: "running" | "stopped" | "paused"; pid?: number };
...
export class proxmox {
	...
	async lxcCreate(node: string, spec: z.infer<typeof lxcCreateSchema>): Promise<UpidResponse> {
	...
	async lxcStart(node: string, vmid: number): Promise<UpidResponse> {
		return this.request<UpidResponse>("POST", `/nodes/${node}/lxc/${vmid}/status/start`);
	}

	async lxcStop(node: string, vmid: number): Promise<UpidResponse> {
		return this.request<UpidResponse>("POST", `/nodes/${node}/lxc/${vmid}/status/stop`);
	}
	...
	async lxcSnapshot(node: string, vmid: number, name: string): Promise<UpidResponse> {
		return this.request<UpidResponse>("POST", `/nodes/${node}/lxc/${vmid}/snapshot`, {
			snapname: name,
		});
	}

	async lxcRollback(node: string, vmid: number, name: string): Promise<UpidResponse> {
	...
	async lxcStatus(node: string, vmid: number): Promise<StatusResponse> {
		const resp = await this.request<StatusResponse>("GET", `/nodes/${node}/lxc/${vmid}/status/current`);
	...
	async vmPower(
		...
		return this.request<UpidResponse>("POST", `/nodes/${node}/qemu/${vmid}/status/${action}`);
	}
	...
	async taskWait(
		...
		throw this.makeError("timeout", 0, "Task timeout", `/nodes/${node}/tasks/${upid}/status`);
	}
