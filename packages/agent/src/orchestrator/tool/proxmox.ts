...
const inputSchema = z
  .object({
    ...
    powerAction: z
      .enum(["start", "stop", "reset", "shutdown", "suspend", "resume"])
      .optional(),
...
const outputSchema = z.discriminatedUnion("action", [
  z.object({
    action: z.literal("lxc_create"),
    ok: z.literal(true),
    upid: z.string(),
  }),
  z.object({
    action: z.literal("lxc_start"),
    ok: z.literal(true),
    upid: z.string(),
  }),
  z.object({
    action: z.literal("lxc_stop"),
    ok: z.literal(true),
    upid: z.string(),
  }),
  z.object({
    action: z.literal("lxc_destroy"),
    ok: z.literal(true),
    upid: z.string(),
  }),
  z.object({
    action: z.literal("lxc_snapshot"),
    ok: z.literal(true),
    upid: z.string(),
  }),
  z.object({
    action: z.literal("lxc_rollback"),
    ok: z.literal(true),
    upid: z.string(),
  }),
  z.object({
...
function makeClient(): proxmox {
  ...
  if (!(host && tokenId && tokenSecret)) {
    throw new Error(
      "Missing PROXMOX_HOST, PROXMOX_TOKEN_ID, or PROXMOX_TOKEN_SECRET"
    );
  }
...
export const toolProxmox = {
  name: "proxmox",
  description:
    "Manage Proxmox LXC containers and VMs with lifecycle, power, and snapshot operations",
  inputSchema,
  ...
  execute: async ({
    ...
    try {
      ...
      switch (input.action) {
        case "lxc_create": {
          if (
            !(input.vmid && input.hostname && input.ostemplate && input.rootfs)
          ) {
            return {
              ok: false,
              code: "validation",
              message:
                "Missing required fields: vmid, hostname, ostemplate, rootfs",
            };
          }
        ...
        case "lxc_snapshot": {
          if (!(input.vmid && input.snapshot)) {
            return {
              ok: false,
              code: "validation",
              message: "Missing vmid or snapshot",
            };
          }
          const result = await client.lxcSnapshot(
            node,
            input.vmid,
            input.snapshot
          );
        ...
        case "lxc_rollback": {
          ...
          const result = await client.lxcRollback(
            node,
            input.vmid,
            input.snapshot
          );
        ...
        case "vm_power": {
          if (!(input.vmid && input.powerAction)) {
            return {
              ok: false,
              code: "validation",
              message: "Missing vmid or powerAction",
            };
          }
          const result = await client.vmPower(
            node,
            input.vmid,
            input.powerAction
          );
