...
describe("Emergent Behavior & Entity Linking", () => {
  beforeAll(async () => {
    ...
    const labels = ["React", "Frontend", "Coding", "Python", "Security", "Kali", "AI", "Transformer", "DeepA", "DeepB", "DeepC", "DeepD"];
    ...
    try {
        ...
        embeddings = labels.map(() => Array.from({length: 1024}, () => Math.random() - 0.5));
    }
    ...
    const getId = (label: string) => nodeMap.get(`ontology:h:${label.toLowerCase()}`)!.id;
    
    const edges = [
        // React -> Frontend -> Coding
        { resource: "ontology", hash: "e:r-f", fromId: getId("React"), toId: getId("Frontend"), kind: "is_a" },
        { resource: "ontology", hash: "e:f-c", fromId: getId("Frontend"), toId: getId("Coding"), kind: "is_a" },
        
        // Python -> Coding
        { resource: "ontology", hash: "e:p-c", fromId: getId("Python"), toId: getId("Coding"), kind: "is_a" },
        
        // Kali -> Security
        { resource: "ontology", hash: "e:k-s", fromId: getId("Kali"), toId: getId("Security"), kind: "tool_of" },
        
        // Transformer -> AI
        { resource: "ontology", hash: "e:t-a", fromId: getId("Transformer"), toId: getId("AI"), kind: "concept_in" },
        
        // Deep Chain: DeepA -> DeepB -> DeepC -> DeepD -> Coding
        { resource: "ontology", hash: "e:da-db", fromId: getId("DeepA"), toId: getId("DeepB"), kind: "next" },
        { resource: "ontology", hash: "e:db-dc", fromId: getId("DeepB"), toId: getId("DeepC"), kind: "next" },
        { resource: "ontology", hash: "e:dc-dd", fromId: getId("DeepC"), toId: getId("DeepD"), kind: "next" },
        { resource: "ontology", hash: "e:dd-c", fromId: getId("DeepD"), toId: getId("Coding"), kind: "next" },
    ];
  ...
  it("should link 'React' to 'Coding'", async () => {
    const result = await linkEntities([{ role: "user", content: "I use React." }]);
    expect(result.domains).toContain("Coding");
  });

  it("should link 'Python' to 'Coding'", async () => {
    const result = await linkEntities([{ role: "user", content: "Python script" }]);
    expect(result.domains).toContain("Coding");
  });

  // 2. Ambiguous / Multi-domain
  it("should link 'Python' and 'Kali' to 'Coding' and 'Security'", async () => {
    const result = await linkEntities([{ role: "user", content: "Using Python on Kali Linux" }]);
    expect(result.domains).toContain("Coding");
  ...
  it("should return empty for irrelevant input", async () => {
    ...
    if (result.domains.length > 0) {
        console.warn("Irrelevant input matched (false positive):", result.domains);
    }
  });
  ...
  it("should NOT find Coding via too-long path (DeepA)", async () => {
      ...
      const result = await linkEntities([{ role: "user", content: "DeepA analysis" }]);
      expect(result.domains).not.toContain("Coding");
  });
  ...
  it("should find Coding via valid path depth (DeepC)", async () => {
      ...
      const result = await linkEntities([{ role: "user", content: "DeepC analysis" }]);
      expect(result.domains).toContain("Coding");
  });
  ...
  it("should complete linking within 500ms (budget)", async () => {
      ...
      await linkEntities([{ role: "user", content: "React and Python integration" }]);
  ...
  it("should handle lowercase input 'react'", async () => {
      const result = await linkEntities([{ role: "user", content: "i use react daily" }]);
      expect(result.domains).toContain("Coding");
  });
  ...
  it("should ignore common stop words/noise", async () => {
     const result = await linkEntities([{ role: "user", content: "the a is of" }]);
     expect(result.domains).toHaveLength(0);
  });
  
  // 11. Multiple entities in one message
  it("should detect multiple entities", async () => {
      const result = await linkEntities([{ role: "user", content: "React running on Kali" }]);
      expect(result.domains).toContain("Coding");
      expect(result.domains).toContain("Security");
  });
