...
type CodexStreamEvent =
  ...
  | { type: "complete"; result: string; artifacts?: Array<{ path: string; kind: string }> }
  | { type: "error"; message: string };

const codexProcedures = {
  run: authedProcedure.input(codexRunInputSchema).mutation(async ({ input }) => {
    const chunks: string[] = [];
    ...
    await toolCodex.execute({
      ...
      writer: {
        write: (chunk: unknown) => {
          const event = chunk as { type?: string; text?: string; event?: AlfredCodexEvent };
