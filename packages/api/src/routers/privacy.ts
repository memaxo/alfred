...
export const privacyRouter = router({
  facts: authedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (params.embedding) {
        const matches = await userRepo.searchFacts(session.user.id, params.embedding, params.limit, params.threshold);
      ...
      const listed = repo.listFacts ? await repo.listFacts(session.user.id, params.limit, params.offset) : [];
      return Array.isArray(listed) ? listed : [];
    }),

  deleteFact: authedProcedure
    .use(requirePolicy("privacy.purge", (input, ctx) => mapPrivacyResource(input, ctx)))
  ...
  events: authedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const events = await userRepo.getEvents(session.user.id, params.type, params.limit, params.offset);
