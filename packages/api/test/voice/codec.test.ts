...
function generatePcmBase64(durationMs = 250, frequency = 440) {
  ...
  for (let i = 0; i < samples; i++) {
    ...
    buffer.writeInt16LE(Math.round(clamped * 32_767), i * 2);
  }
...
describe.skipIf(shouldSkip)("voice codec helpers", () => {
  it("detects ffmpeg availability upfront", () => {
    if (ffmpegAvailable) {
      expect(() => ensureFfmpegAvailable()).not.toThrow();
      return;
    }
    expect(() => ensureFfmpegAvailable()).toThrow();
  });

  it.skipIf(!ffmpegAvailable)("decodes webm/opus input into PCM", async () => {
  ...
  it.skipIf(!ffmpegAvailable)(
    "encodes PCM into mp3, opus, and wav",
    async () => {
      const pcmBase64 = generatePcmBase64();
      const mp3 = await encodeFromPCM16({
      ...
      const wav = await encodeFromPCM16({
        audioBase64: pcmBase64,
        format: "wav",
      });
  ...
  it.skipIf(!ffmpegAvailable)("round-trips PCM -> mp3 -> PCM", async () => {
