...
export const capturing = (input, conf) => ({
    _: "capturing",
    input,
    confidence: confidence(conf),
    started: timestamp(Date.now()),
});
export const thinking = (about, depth = 1, traces) => ({
    _: "thinking",
    about,
...
export const deciding = (options, criteria) => ({
    _: "deciding",
    options,
...
export const executing = (plan, auto) => ({
    _: "executing",
    plan,
    step: 0,
    auto,
...
export const reflecting = (outcome, expected, actual) => ({
    _: "reflecting",
    outcome,
    expected,
    actual,
...
export const updateAutonomy = (current, evidence) => {
...
const bayesianUpdate = (priorLevel, priorConfidence, evidence) => {
 ...
 */
export const evaluateReasoningQuality = (traces, outcome) => {
    if (traces.length === 0) {
        return { _: "feedback", positive: false, strength: 0.3 };
    }
    const avgLength = traces.reduce((sum, text) => sum + text.length, 0) / traces.length;
    const hasDecisionPoints = traces.some((text) => /considering|choosing|selecting|decided/i.test(text));
    const hasAlternatives = traces.some((text) => /however|alternatively|instead|but|though/i.test(text));
    const hasCausalReasoning = traces.some((text) => /because|therefore|thus|leads to|causes/i.test(text));
    let score = 0.5;
    if (avgLength > 50) {
        score += 0.1;
    }
...
// Constraint checking
export const meetsConstraints = (auto, action) => {
...
export const canInterrupt = (state) => state._ === "thinking" || state._ === "deciding";
export const requiresInput = (state) => state._ === "idle" || state._ === "reflecting";
export const isExecuting = (state) => state._ === "executing";
...
export const isStale = (state, threshold = 30_000) => duration(state) > threshold;
...
export const statusFocus = (state) => {
    if (!state) {
        return { _: "idle" };
    }
    return {
...
export const startFocus = (current, params) => {
...
export const stopFocus = (current) => {
    if (!current) {
        return { _: "idle" };
    }
