...
export type CognitiveState = 
  ...
  | { _: "capturing"; input: string; confidence: Confidence; started: Timestamp }
  | { _: "thinking"; about: string; depth: number; paths: Path[]; started: Timestamp }
  | { _: "deciding"; options: Decision[]; criteria: Criteria; weights: number[]; deadline: Timestamp }
  | { _: "executing"; plan: Plan; step: number; auto: AutonomyGradient; started: Timestamp }
  | { _: "reflecting"; outcome: Outcome; expected: string; actual: string; error: number }
...
export const deciding = (options: Decision[], criteria?: Criteria): CognitiveState => ({
  _: "deciding",
  options,
...
export const executing = (plan: Plan, auto: AutonomyGradient): CognitiveState => ({
  _: "executing",
  plan,
  step: 0,
  auto,
...
export const reflecting = (outcome: Outcome, expected: string, actual: string): CognitiveState => ({
  _: "reflecting",
  outcome,
  expected,
  actual,
...
export const updateAutonomy = (current: AutonomyGradient, evidence: Evidence): AutonomyGradient => {
  const prior = current.level
  const priorConfidence = current.confidence
  
  // Bayesian update based on evidence
  const [newLevel, newConfidence] = bayesianUpdate(prior, priorConfidence, evidence)
...
// Constraint checking
export const meetsConstraints = (auto: AutonomyGradient, action: string): boolean => {
  for (const constraint of auto.constraints) {
    switch (constraint._) {
      ...
      case "scope":
        ...
        if (constraint.allowed.length > 0 && !constraint.allowed.includes(action)) return false
        break
      case "confidence":
...
// State predicates
export const isActive = (state: CognitiveState): boolean => 
  state._ !== "idle"
...
export const duration = (state: CognitiveState): number => {
  const now = Date.now()
  switch (state._) {
    case "idle": return now - state.since
    case "capturing": return now - state.started
    case "thinking": return now - state.started
    case "deciding": return state.deadline - now
    case "executing": return now - state.started
    case "reflecting": return 0
  }
}

export const isStale = (state: CognitiveState, threshold = 30000): boolean =>
...
export const statusFocus = (state: FocusState | null | undefined): FocusState => {
  if (!state) {
    return { _: "idle" }
  }
  return {
...
export const stopFocus = (current: FocusState | null | undefined): FocusState => {
  if (!current) {
    return { _: "idle" }
  }
...
export const updateFocus = (
  ...
  const duration = params.durationMin !== undefined ? clampDuration(params.durationMin) : base.duration
