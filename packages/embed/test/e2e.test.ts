...
describePgModel("E2E: Note -> RAG -> Embed Flow", () => {
  ...
  test("complete flow: ingest -> embed -> retrieve", async () => {
    ...
    const noteContent =
      "ALFRED uses local KaLM-Embedding model for privacy-preserving embeddings with 3840 dimensions.";
    ...
    if (chunks.length > 0) {
      ...
      console.log(
        `[e2e-test] ✓ Content preview: ${topChunk.content.slice(0, 80)}...`
      );
    }
  ...
  test("batch embedding maintains consistency", async () => {
    ...
    for (let i = 0; i < notes.length; i++) {
      ...
      const diff = individual[i].reduce(
        (sum, val, idx) => sum + Math.abs(val - batched[i][idx]),
        0
      );
    ...
    console.log(
      "[e2e-test] ✓ Batch and individual embeddings are consistent (1024 dims)"
    );
  }, 120_000);
  ...
  test("semantic similarity detection", async () => {
    ...
    const docs = [
      {
        source: `test:doc1:${Date.now()}`,
        content:
          "Python is a popular programming language for data science and machine learning.",
      },
      {
        source: `test:doc2:${Date.now()}`,
        content:
          "JavaScript is widely used for web development and frontend applications.",
      },
      {
        source: `test:doc3:${Date.now()}`,
        content:
          "PyTorch and TensorFlow are deep learning frameworks built with Python.",
      },
    ];
    ...
    console.log(
      "[e2e-test] ✓ Semantic search successfully distinguishes topics"
    );
  }, 150_000);

  test("RAG enrichment populates memory_nodes when enabled", async () => {
