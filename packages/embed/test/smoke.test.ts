...
describe("Embedding Service - Smoke Test", () => {
  ...
  testModel(
    "service initializes and returns valid embeddings",
    async () => {
      ...
      console.log(
        `[smoke-test] ✓ Generated ${EMBEDDING_DIM}-dimensional embedding (MRL truncated)`
      );
    },
    90_000
  ); // 90s timeout for first run (downloads model if needed)
  ...
  testModel(
    ...
    async () => {
      ...
      console.log(
        `[smoke-test] ✓ Vector normalized (L2 norm: ${norm.toFixed(4)})`
      );
    },
    60_000
  );
