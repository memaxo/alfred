...
export async function buildHistoryContext(options) {
    const sourceLabel = options.source ?? "history";
    return withBudget(`build_history_context_${sourceLabel}`, 10, async () => {
        const messages = (Array.isArray(options.messages) ? options.messages : []).filter((m) => !!m);
        ...
        const budget = resolveBudget(options, modelContext, options.system ? estimator.estimate(options.system) : 0);
        const tokensByIndex = messages.map((message, index) => estimateMessageTokens(estimator, message, index));
...
function resolveBudget(options, modelContext, systemTokens) {
    ...
    let ratio = typeof overrides.historyRatio === "number"
    ...
    const minSystemReserveTokens = overrides.minSystemReserveTokens ??
        envSystemReserve ??
        DEFAULT_MIN_SYSTEM_RESERVE;
    const minHeadroomTokens = overrides.minHeadroomTokens ?? envHeadroom ?? DEFAULT_MIN_HEADROOM;
    const reservedToolingTokens = overrides.reservedToolingTokens ?? DEFAULT_RESERVED_TOOLING;
...
function estimateMessageTokens(estimator, message, _index) {
...
function normalizeForceKeep(forceKeep) {
    if (!forceKeep) {
        return new Set();
    }
...
function findLastIndexByRole(messages, role, startIndex) {
    for (let index = typeof startIndex === "number" ? startIndex : messages.length - 1; index >= 0; index -= 1) {
...
function mapToolGroups(_messages, toolChains) {
    const map = new Map();
...
//# sourceMappingURL=history-context.js.map