...
export type RadarrSystemStatus = z.infer<typeof systemStatusSchema>;

export class Radarr {
  private readonly cfg: Required<RadarrCfg>;

  constructor(cfg: RadarrCfg) {
    this.cfg = {
      baseUrl: cfg.baseUrl.replace(/\/$/, ""),
      apiKey: cfg.apiKey,
      apiPrefix: cfg.apiPrefix ?? "/api/v3",
  ...
  async systemStatus(): Promise<RadarrSystemStatus> {
    const res = await this.request<unknown>("GET", "/system/status");
    return systemStatusSchema.parse(res);
  }
