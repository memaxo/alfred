 ...
 */
export const extract = (text, source) => {
    const facts = [];
    const causality = [];
...
export const toKnowledge = (result) => {
    const list = [];
    const seen = new Set();
    const insert = (item) => {
    ...
    for (const c of result.causality) {
        ...
        insert(insight([causeId, effectId], `${c.cause} causes ${c.effect}`, c.confidence));
    }
    return list;
};
/**
 ...
 */
const detectContradiction = (s1, s2) => {
 ...
 */
export const inferPattern = (examples, minSupport = 0.7) => {
 ...
 */
export const extractTemporal = (text) => {
    const temporal = [];
 ...
 */
export const extractReasoning = (text, context) => {
 ...
 */
export const enrichReasoningContext = (entries, context) => {
