...
class RTreeNode {
  ...
  constructor(
     leaf,
     parent
  ) {;this.leaf = leaf;this.parent = parent;RTreeNode.prototype.__init.call(this);}
}
...
export class RTreeND {
    ...
    __init2() {this.leafLookup = new Map()}
    __init3() {this.entryLookup = new Map()}
  ...
  ) {;this.dim = dim;this.maxEntries = maxEntries;RTreeND.prototype.__init2.call(this);RTreeND.prototype.__init3.call(this);
    if (this.dim <= 0) {
  ...
  nearestK(
    ...
    const worst = () => {
      ...
      return _nullishCoalesce(_optionalChain([last, 'optionalAccess', _ => _.dist]), () => ( Number.POSITIVE_INFINITY));
    };
  ...
  remove(id) {
   ...
   normalizeRect(rect) {
    ...
    for (let i = 0; i < this.dim; i++) {
      const minVal = _nullishCoalesce(rect.min[i], () => ( 0));
      const maxVal = _nullishCoalesce(rect.max[i], () => ( 0));
   ...
   chooseLeaf(node, rect) {
    ...
    if (!_optionalChain([best, 'optionalAccess', _2 => _2.child])) {
   ...
   adjustTree(node) {
    ...
    if (!this.root.leaf && this.root.entries.length === 1) {
      const child = _optionalChain([this, 'access', _3 => _3.root, 'access', _4 => _4.entries, 'access', _5 => _5[0], 'optionalAccess', _6 => _6.child]);
      if (child) {
        child.parent = null;
        this.root = child;
      }
   ...
   pickSeeds(entries) {
    ...
    for (let i = 0; i < entries.length - 1; i++) {
      for (let j = i + 1; j < entries.length; j++) {
        const rect = this.combine(_optionalChain([entries, 'access', _7 => _7[i], 'optionalAccess', _8 => _8.rect]), _optionalChain([entries, 'access', _9 => _9[j], 'optionalAccess', _10 => _10.rect]));
        const waste =
          this.measure(rect) -
          this.measure(_optionalChain([entries, 'access', _11 => _11[i], 'optionalAccess', _12 => _12.rect])) -
          this.measure(_optionalChain([entries, 'access', _13 => _13[j], 'optionalAccess', _14 => _14.rect]));
        if (waste > maxWaste) {
          const e1 = entries[i];
          const e2 = entries[j];
          if (e1 && e2) {
   ...
   groupRect(entries) {
    ...
    let rect = _optionalChain([entries, 'access', _15 => _15[0], 'optionalAccess', _16 => _16.rect]);
    ...
    for (let i = 1; i < entries.length; i++) {
      rect = this.combine(rect, _optionalChain([entries, 'access', _17 => _17[i], 'optionalAccess', _18 => _18.rect]));
    }
    return rect;
  }
   ...
   computeNodeRect(node) {
    ...
    const firstRect = _optionalChain([node, 'access', _19 => _19.entries, 'access', _20 => _20[0], 'optionalAccess', _21 => _21.rect]);
    if (!firstRect) {
      const min = new Float32Array(this.dim);
      const max = new Float32Array(this.dim);
      return { min, max };
    ...
    for (let i = 1; i < node.entries.length; i++) {
      ...
      for (let d = 0; d < this.dim; d++) {
        const entryMin = _nullishCoalesce(entry.rect.min[d], () => ( 0));
        const entryMax = _nullishCoalesce(entry.rect.max[d], () => ( 0));
        const currentMin = _nullishCoalesce(min[d], () => ( 0));
        const currentMax = _nullishCoalesce(max[d], () => ( 0));
   ...
   condenseTree(start) {
    ...
    if (!this.root.leaf && this.root.entries.length === 1) {
      const child = _optionalChain([this, 'access', _22 => _22.root, 'access', _23 => _23.entries, 'access', _24 => _24[0], 'optionalAccess', _25 => _25.child]);
      if (child) {
        child.parent = null;
        this.root = child;
      }
   ...
   combine(
    ...
    if (!a) return _nullishCoalesce(b, () => ( { min, max }));
    if (!b) return a;

    for (let i = 0; i < this.dim; i++) {
      const aMin = _nullishCoalesce(a.min[i], () => ( 0));
      const bMin = _nullishCoalesce(b.min[i], () => ( 0));
      const aMax = _nullishCoalesce(a.max[i], () => ( 0));
      const bMax = _nullishCoalesce(b.max[i], () => ( 0));
   ...
   distancePointRect(point, rect) {
    let sum = 0;
    for (let i = 0; i < this.dim; i++) {
      const p = _nullishCoalesce(point[i], () => ( 0));
      const min = _nullishCoalesce(rect.min[i], () => ( 0));
      const max = _nullishCoalesce(rect.max[i], () => ( 0));
      let delta = 0;
      if (p < min) {
        delta = min - p;
      } else if (p > max) {
