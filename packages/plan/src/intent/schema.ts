...
export const clarificationQuestionSchema = z.object({
  ...
  options: z.array(z.string()).optional().describe("Multiple choice options for the user"),
...
export const workflowIntentSchema: z.ZodType<any> = z.object({
  ...
  userId: z.string(),
  timestamp: z.date().or(z.string().datetime().transform((s) => new Date(s))),
  context: z.object({
...
export const intentParserOutputSchema = z.object({
  ...
  ambiguity: z.object({
    score: z.number().min(0).max(1).describe("Ambiguity score (0.0-1.0), higher = more ambiguous"),
    questions: z.array(z.object({
      question: z.string().describe("Specific clarification question"),
      options: z.array(z.string()).optional().describe("Suggested multiple choice options"),
    })).describe("Questions to resolve ambiguity"),
  }).describe("Ambiguity detection results"),
  multiIntent: z.object({
    split: z.boolean().describe("Whether this should be split into multiple intents"),
    parts: z.array(z.string()).describe("The individual intent descriptions if split"),
