export const resetCognitiveTables = async (): Promise<void> => {
  ...
  await db.delete(cognitiveEvents).where(like(cognitiveEvents.streamId, testPrefix));
  await db.delete(cognitiveSnapshots).where(
    like(cognitiveSnapshots.streamId, testPrefix)
  );
};
