...
mock.module("../src/context", () => ({
  ContextBuilder: class {
    build() { return Promise.resolve({ bundle: {} }); }
  }
}));
...
describe("Orchestrator Hydration", () => {
  it("skips completed waves from history", async () => {
    ...
    // Should skip wave_0
    const skipEvent = events.find(e => e.type === "notice" && e.message === "wave_wave_0_skipped_already_completed");
    expect(skipEvent).toBeDefined();

    // Should run wave_1
    const startEvent = events.find(e => e.type === "notice" && e.message === "wave_wave_1_start");
  ...
  it("resumes failed waves from history (does NOT skip)", async () => {
    ...
    const startEvent = events.find(e => e.type === "notice" && e.message === "wave_wave_0_start");
    expect(startEvent).toBeDefined();
  });
});
