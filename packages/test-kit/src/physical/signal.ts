...
export class SyntheticSignal {
   ...
   */
  static sine(hz: number, durationMs: number, sampleRate = 16000): Buffer {
    ...
    for (let i = 0; i < numSamples; i++) {
      ...
      const val = Math.max(-32768, Math.min(32767, amplitude * 32767));
  ...
  static silence(durationMs: number, sampleRate = 16000): Buffer {
    const numSamples = Math.floor((sampleRate * durationMs) / 1000);
