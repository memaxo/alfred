...
interface NativeAudioModule {
	...
	Sound: {
		...
		): Promise<{ sound: { playAsync(): Promise<void>; unloadAsync(): Promise<void> } }>;
	};
...
export function createNativeAdapter(deps: NativeDeps): PlatformAdapter {
	...
	async function configureSession(opts: { mode: string; background?: boolean }): Promise<void> {
	...
	async function stopCapture(): Promise<{ mimeType: string; audioBase64: string } | null> {
		if (!recording) {
			return null;
		}
		...
		const audioBase64 = await deps.FileSystem.readAsStringAsync(uri, { encoding: "base64" });
	...
	async function play(base64: string, mimeType: string): Promise<void> {
		...
		await deps.FileSystem.writeAsStringAsync(filename, base64, { encoding: "base64" });
		try {
