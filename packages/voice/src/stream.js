...
export class VoiceStreamClient {
    ...
    ensureConnection() {
        ...
        this.connectionPromise = new Promise((resolve, reject) => {
            try {
                const ws = createSocket(this.options.url, this.options.protocols, this.options.headers);
                this.socket = ws;
                if ("binaryType" in ws) {
                    try {
                ...
                ws.onclose = (event) => {
                    ...
                    this.rejectPendingSession(new Error(event.reason || "voice_stream_socket_closed"));
                };
            }
            catch (error) {
                this.connectionPromise = null;
    ...
    handleServerEvent(event) {
        switch (event.type) {
            ...
            case "error":
                ...
                if (!this.sessionId) {
                    this.rejectPendingSession(new Error(event.message || "voice_stream_error"));
                }
                return;
            case "pong":
                return;
