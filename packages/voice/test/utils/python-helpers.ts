import { accessSync, constants as fsConstants, mkdirSync, rmSync, writeFileSync, chmodSync } from "node:fs";
...
export async function hasUv(): Promise<boolean> {
  try {
    const findCmd = process.platform === "win32" ? ["where", "uv"] : ["which", "uv"];
 ...
 */
export async function hasPythonDependencies(pythonCmd: string[]): Promise<boolean> {
  try {
 ...
 */
export function saveEnvVars(keys: string[]): Record<string, string | undefined> {
 ...
 */
export function createTempDir(prefix: string = "alfred-voice-test-"): string {
